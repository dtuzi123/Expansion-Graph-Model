...
for taskIndex in range(taskCount):
    if taskIndex == 0:
        ...
        auxilaryX = np.concatenate((CaltechTesting,omnistTestingSet,fashionTest),axis=0)
    elif taskIndex == 3:
...
f = open("results/CrossDomain_SourceRisk" + str(0) + ".txt", "w", encoding="utf-8")
...
f = open("results/CrossDomain_TargetRisk" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
...
f = open("results/CrossDomain_Discrepancy" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
...
f = open("results/CrossDomain_KLDivergence" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
