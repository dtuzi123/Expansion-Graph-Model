...
if args.stochastic_layers == 1:
    ...
    baseVAEModel = iwae1.IWAE(n_hidden[0], n_latent[0])
    IWVAEModell = iwae1.IWAE(n_hidden[0], n_latent[0])
    IWVAEModel2 = iwae1.IWAE(n_hidden[0], n_latent[0])
else:
    n_latent = [100, 50]
    n_hidden = [200, 100]
...
def EvaluationByModel(model,test):
...
for taskIndex in range(taskCount):
    ...
    if taskIndex != 0:
        ...
        for i in range(myCount):
            ...
            generatedImages = baseVAEModel.generate_samples(z)
    ...
    for epoch in range(epochs):
        ...
        for idx in range(myCount):
            ...
            beta = 1.0
            res = baseVAEModel.train_step(batchImages, n_samples, beta, optimizer, objective="vae_elbo")
            res1 = IWVAEModell.train_step(batchImages, 5, beta, optimizer, objective=objective)
            res2 = IWVAEModel2.train_step(batchImages, n_samples, beta, optimizer, objective=objective)
        ...
        #Calculate measures
        if epoch % 10 == 0:
            baseVae_caltechScore = EvaluationByModel(baseVAEModel,CaltechTesting)
            baseVae_fashionScore = EvaluationByModel(baseVAEModel,fashionTest)
            baseVae_score = (baseVae_caltechScore + baseVae_fashionScore)/2.0
            vaeArr.append(baseVae_score)

            IWVAE1_caltechScore = EvaluationByModel(IWVAEModell, CaltechTesting)
            IWVAE1_fashionScore = EvaluationByModel(IWVAEModell, fashionTest)
            IWVAE1_score = (IWVAE1_caltechScore + IWVAE1_fashionScore) / 2.0
            iwvaeArr1.append(IWVAE1_score)

            IWVAE2_caltechScore = EvaluationByModel(IWVAEModel2, CaltechTesting)
            IWVAE2_fashionScore = EvaluationByModel(IWVAEModel2, fashionTest)
            IWVAE2_score = (IWVAE2_caltechScore + IWVAE2_fashionScore) / 2.0
            iwvaeArr2.append(IWVAE2_score)
...
lossArr1 = np.array(vaeArr).astype('str')
f = open("results/IELBO_TwoModels_VAE_2" + str(0) + ".txt", "w", encoding="utf-8")
...
lossArr1 = np.array(iwvaeArr1).astype('str')
f = open("results/IELBO_TwoModels_IWVAE1_2" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
...
lossArr1 = np.array(iwvaeArr2).astype('str')
f = open("results/IELBO_TwoModels_IWVAE2_2" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
