...
for taskIndex in range(taskCount):
    if taskIndex == 0:
        ...
        currentX = mnistTrain
        auxilaryX = np.concatenate((CaltechTesting,fashionTest,mnistTest),axis=0)
    elif taskIndex == 3:
...
f = open("results/CrossDomain_SourceRisk_Inverse" + str(0) + ".txt", "w", encoding="utf-8")
...
f = open("results/CrossDomain_TargetRisk_Inverse" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
...
f = open("results/CrossDomain_Discrepancy_Inverse" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
...
f = open("results/CrossDomain_KLDivergence_Inverse" + str(0) + ".txt", "w", encoding="utf-8")
for i in range(np.shape(lossArr1)[0]):
    f.writelines(lossArr1[i])
    f.writelines('\n')
f.flush()
