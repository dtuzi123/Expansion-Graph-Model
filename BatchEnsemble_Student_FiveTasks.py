...
class LifeLone_MNIST(object):
    ...
    def build_model(self):
        ...
        z_mean2, z_log_sigma_sq12,features2 = Encoder_SVHN_Specific(self.Shared_Encoder, "encoder2", batch_size=64, reuse=False)
        continous_variables2 = z_mean2 + z_log_sigma_sq12 * tf.random_normal(tf.shape(z_mean2), 0, 1, dtype=tf.float32)

        z_mean3, z_log_sigma_sq13,features3 = Encoder_SVHN_Specific(self.Shared_Encoder, "encoder3", batch_size=64, reuse=False)
        continous_variables3 = z_mean3 + z_log_sigma_sq13 * tf.random_normal(tf.shape(z_mean3), 0, 1, dtype=tf.float32)

        z_mean4, z_log_sigma_sq14, features4 = Encoder_SVHN_Specific(self.Shared_Encoder, "encoder4", batch_size=64,reuse=False)
        continous_variables4 = z_mean4 + z_log_sigma_sq14 * tf.random_normal(tf.shape(z_mean4), 0, 1, dtype=tf.float32)

        z_mean5, z_log_sigma_sq15, features5 = Encoder_SVHN_Specific(self.Shared_Encoder, "encoder5", batch_size=64,reuse=False)
        continous_variables5 = z_mean5 + z_log_sigma_sq15 * tf.random_normal(tf.shape(z_mean5), 0, 1, dtype=tf.float32)

        z_mean6, z_log_sigma_sq16, features6 = Encoder_SVHN_Specific(self.Shared_Encoder, "encoder6", batch_size=64,reuse=False)
        continous_variables6 = z_mean6 + z_log_sigma_sq16 * tf.random_normal(tf.shape(z_mean6), 0, 1, dtype=tf.float32)

        z_meanStu, z_log_sigma_sq1Stu = Encoder_SVHN(self.inputs,"Student_Encoder", batch_size=64,reuse=False)
        continous_variablesStu = z_meanStu + z_log_sigma_sq1Stu * tf.random_normal(tf.shape(z_meanStu), 0, 1, dtype=tf.float32)

        VAE_student = Generator_SVHN("Student_Generator", continous_variablesStu, reuse=False)
        KL_divergenceStu = 0.5 * tf.reduce_sum(
            tf.square(z_meanStu) + tf.square(z_log_sigma_sq1Stu) - tf.log(1e-8 + tf.square(z_log_sigma_sq1Stu)) - 1, 1)
        KL_divergenceStu = tf.reduce_mean(KL_divergenceStu)
        reconstruction_lossStu = tf.reduce_mean(tf.reduce_sum(tf.square(VAE_student - self.inputs), [1, 2, 3]))

        self.Stu_LogLikelihood = reconstruction_lossStu+KL_divergenceStu
        self.Reco = VAE_student
        self.RecoLoss = reconstruction_lossStu

        code1 = continous_variables1
        code2 = continous_variables2
        code3 = continous_variables3
        code4 = continous_variables4
        code5 = continous_variables5
        code6 = continous_variables6

        self.Shared_Decoder1 = Generator_SharedSVHN2_BatchEnsemble("VAE_Generator_Shared", code1, reuse=False)
        VAE1 = Generator_SubSVHN2_BatchEnsemble("VAE_Generator1", self.Shared_Decoder1, reuse=False)

        RandomDecoder = Generator_SharedSVHN2_BatchEnsemble("VAE_Generator_Shared", self.z, reuse=True)
        self.VAE1_generation = Generator_SubSVHN2_BatchEnsemble("VAE_Generator1", RandomDecoder, reuse=True)

        self.Shared_Decoder2 = Generator_SharedSVHN2_BatchEnsemble("VAE_Generator_Shared", code2, reuse=True)
        VAE2 = Generator_SubSVHN2_BatchEnsemble("VAE_Generator2", self.Shared_Decoder2, reuse=False)
        self.VAE2_generation = Generator_SubSVHN2_BatchEnsemble("VAE_Generator2", RandomDecoder, reuse=True)

        self.Shared_Decoder3 = Generator_SharedSVHN2_BatchEnsemble("VAE_Generator_Shared", code3, reuse=True)
        VAE3 = Generator_SubSVHN2_BatchEnsemble("VAE_Generator3", self.Shared_Decoder3, reuse=False)
        self.VAE3_generation = Generator_SubSVHN2_BatchEnsemble("VAE_Generator3", RandomDecoder, reuse=True)

        self.Shared_Decoder4 = Generator_SharedSVHN2_BatchEnsemble("VAE_Generator_Shared", code4, reuse=True)
        VAE4 = Generator_SubSVHN2_BatchEnsemble("VAE_Generator4", self.Shared_Decoder4, reuse=False)
        self.VAE4_generation = Generator_SubSVHN2_BatchEnsemble("VAE_Generator4", RandomDecoder, reuse=True)

        self.Shared_Decoder5 = Generator_SharedSVHN2_BatchEnsemble("VAE_Generator_Shared", code5, reuse=True)
        VAE5 = Generator_SubSVHN2_BatchEnsemble("VAE_Generator5", self.Shared_Decoder5, reuse=False)
        self.VAE5_generation = Generator_SubSVHN2_BatchEnsemble("VAE_Generator5", RandomDecoder, reuse=True)

        self.Shared_Decoder6 = Generator_SharedSVHN2_BatchEnsemble("VAE_Generator_Shared", code6, reuse=True)
        VAE6 = Generator_SubSVHN2_BatchEnsemble("VAE_Generator6", self.Shared_Decoder6, reuse=False)
        self.VAE6_generation = Generator_SubSVHN2_BatchEnsemble("VAE_Generator6", RandomDecoder, reuse=True)

        KL_divergence1 = 0.5 * tf.reduce_sum(tf.square(z_mean1) + tf.square(z_log_sigma_sq1) - tf.log(1e-8 + tf.square(z_log_sigma_sq1)) - 1,1)
        KL_divergence1 = tf.reduce_mean(KL_divergence1)

        KL_divergence2 = 0.5 * tf.reduce_sum(tf.square(z_mean2) + tf.square(z_log_sigma_sq12) - tf.log(1e-8 + tf.square(z_log_sigma_sq12)) - 1, 1)
        KL_divergence2 = tf.reduce_mean(KL_divergence2)

        KL_divergence3 = 0.5 * tf.reduce_sum(tf.square(z_mean3) + tf.square(z_log_sigma_sq13) - tf.log(1e-8 + tf.square(z_log_sigma_sq13)) - 1, 1)
        KL_divergence3 = tf.reduce_mean(KL_divergence3)

        KL_divergence4 = 0.5 * tf.reduce_sum(tf.square(z_mean4) + tf.square(z_log_sigma_sq14) - tf.log(1e-8 + tf.square(z_log_sigma_sq14)) - 1, 1)
        KL_divergence4 = tf.reduce_mean(KL_divergence4)

        KL_divergence5 = 0.5 * tf.reduce_sum(tf.square(z_mean5) + tf.square(z_log_sigma_sq15) - tf.log(1e-8 + tf.square(z_log_sigma_sq15)) - 1, 1)
        KL_divergence5 = tf.reduce_mean(KL_divergence5)

        KL_divergence6 = 0.5 * tf.reduce_sum(tf.square(z_mean6) + tf.square(z_log_sigma_sq16) - tf.log(1e-8 + tf.square(z_log_sigma_sq16)) - 1, 1)
        KL_divergence6 = tf.reduce_mean(KL_divergence6)

        reconstruction_loss1 = tf.reduce_mean(tf.reduce_sum(tf.square(VAE1 - self.inputs), [1, 2, 3]))
        reconstruction_loss2 = tf.reduce_mean(tf.reduce_sum(tf.square(VAE2 - self.inputs), [1, 2, 3]))
        reconstruction_loss3 = tf.reduce_mean(tf.reduce_sum(tf.square(VAE3 - self.inputs), [1, 2, 3]))
        reconstruction_loss4 = tf.reduce_mean(tf.reduce_sum(tf.square(VAE4 - self.inputs), [1, 2, 3]))
        reconstruction_loss5 = tf.reduce_mean(tf.reduce_sum(tf.square(VAE5 - self.inputs), [1, 2, 3]))
        reconstruction_loss6 = tf.reduce_mean(tf.reduce_sum(tf.square(VAE6 - self.inputs), [1, 2, 3]))

        self.VAE_List.append(VAE1)
        self.VAE_List.append(VAE2)
        self.VAE_List.append(VAE3)
        self.VAE_List.append(VAE4)
        self.VAE_List.append(VAE5)
        self.VAE_List.append(VAE6)

        self.reco1 = reconstruction_loss1
        self.reco2 = reconstruction_loss2
        self.reco3 = reconstruction_loss3
        self.reco4 = reconstruction_loss4
        self.reco5 = reconstruction_loss5
        self.reco6 = reconstruction_loss6

        vaeloss1 = reconstruction_loss1 + KL_divergence1
        self.vaeLoss1 = vaeloss1

        vaeloss2 = reconstruction_loss2 + KL_divergence2
        self.vaeLoss2 = vaeloss2

        vaeloss3 = reconstruction_loss3 + KL_divergence3
        self.vaeLoss3 = vaeloss3

        vaeloss4 = reconstruction_loss4 + KL_divergence4
        self.vaeLoss4 = vaeloss4

        vaeloss5 = reconstruction_loss5 + KL_divergence5
        self.vaeLoss5 = vaeloss5

        vaeloss6 = reconstruction_loss6 + KL_divergence6
        self.vaeLoss6 = vaeloss6
        ...
        T_vars = tf.trainable_variables()
        VAE_encoderShared_vars = [var for var in T_vars if var.name.startswith('encoder_shared')]
        VAE_encoder_vars1 = [var for var in T_vars if var.name.startswith('encoder1')]
        VAE_encoder_vars2 = [var for var in T_vars if var.name.startswith('encoder2')]
        VAE_encoder_vars3 = [var for var in T_vars if var.name.startswith('encoder3')]
        VAE_encoder_vars4 = [var for var in T_vars if var.name.startswith('encoder4')]
        VAE_encoder_vars5 = [var for var in T_vars if var.name.startswith('encoder5')]
        VAE_encoder_vars6 = [var for var in T_vars if var.name.startswith('encoder6')]
        VAE_encoder_vars_Student = [var for var in T_vars if var.name.startswith('Student_Encoder')]

        VAE_generatorShared_vars1 = [var for var in T_vars if var.name.startswith('VAE_Generator_Shared')]
        VAE_generator_vars1 = [var for var in T_vars if var.name.startswith('VAE_Generator1')]
        VAE_generator_vars2 = [var for var in T_vars if var.name.startswith('VAE_Generator2')]
        VAE_generator_vars3 = [var for var in T_vars if var.name.startswith('VAE_Generator3')]
        VAE_generator_vars4 = [var for var in T_vars if var.name.startswith('VAE_Generator4')]
        VAE_generator_vars5 = [var for var in T_vars if var.name.startswith('VAE_Generator5')]
        VAE_generator_vars6 = [var for var in T_vars if var.name.startswith('VAE_Generator6')]
        VAE_generator_vars_Student = [var for var in T_vars if var.name.startswith('Student_Generator')]
        ...
        # optimizers
        with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
            self.vae_optim1 = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.vaeLoss1, var_list=VAE_encoderShared_vars+VAE_encoder_vars1+VAE_generatorShared_vars1+VAE_generator_vars1)
            self.vae_optim2 = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.vaeLoss2, var_list=VAE_encoder_vars2+VAE_generator_vars2)
            self.vae_optim3 = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.vaeLoss3, var_list=VAE_encoder_vars3+VAE_generator_vars3)
            self.vae_optim4 = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.vaeLoss4, var_list=VAE_encoder_vars4+VAE_generator_vars4)
            self.vae_optim5 = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.vaeLoss5, var_list=VAE_encoder_vars5+VAE_generator_vars5)
            self.vae_optim6 = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.vaeLoss6, var_list=VAE_encoder_vars6+VAE_generator_vars6)
            self.vae_student = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.Stu_LogLikelihood, var_list=VAE_generator_vars_Student+VAE_encoder_vars_Student)
    ...
    def Calculate_LOG_ByIndex(self,testX,index):
        p1 = int(np.shape(testX)[0] / self.batch_size)
        #myPro = self.Give_Elbo_byVAE_Reco(index)

        myPro = self.Stu_LogLikelihood
    ...
    def Calculate_Criteria(self,testX,index):
        ...
        log = self.Calculate_LOG_ByIndex(testX,index)

        return mse,ssim,psnr,log
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.saver.restore(sess, 'models/BatchEnsemble_Unsupervised')
            ...
            ims("results/" + "BatchEnsemble_real1" ".png", merge2(self.mnist_test_x[:64], [8, 8]))
            ims("results/" + "BatchEnsemble_reco1" ".png", merge2(a1[:64], [8, 8]))
            print("index:")
            print(index1)

            index2 = 2
            mse2, ssim2, psnr2 = self.Calculate_Criteria(self.svhn_test_x, index2)
            a1 = self.Give_Reconstruction_ByIndex(self.FashionTest_x[0:self.batch_size], index2)
            ims("results/" + "BatchEnsemble_real2" ".png", merge2(self.FashionTest_x[:64], [8, 8]))
            ims("results/" + "BatchEnsemble_real2" ".png", merge2(a1[:64], [8, 8]))
            print("index:")
            print(index2)

            index3 = 3
            mse3, ssim3, psnr3 = self.Calculate_Criteria(self.FashionTest_x, index3)
            a1 = self.Give_Reconstruction_ByIndex(self.svhn_test_x[0:self.batch_size], index3)
            ims("results/" + "BatchEnsemble_real3" ".png", merge2(self.svhn_test_x[:64], [8, 8]))
            ims("results/" + "BatchEnsemble_real3" ".png", merge2(a1[:64], [8, 8]))
            print("index:")
            print(index3)
            ...
            ims("results/" + "BatchEnsemble_real4" ".png", merge2(self.InverseFashionTest_x[:64], [8, 8]))
            ims("results/" + "BatchEnsemble_real4" ".png", merge2(a1[:64], [8, 8]))
            print("index:")
            print(index4)
            ...
            ims("results/" + "BatchEnsemble_real5" ".png", merge2(self.InverseMNISTTest_x[:64], [8, 8]))
            ims("results/" + "BatchEnsemble_real5" ".png", merge2(a1[:64], [8, 8]))
            print("index:")
            print(index5)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    for idx in range(start_batch_id, self.num_batches):
                        ...
                        if taskIndex == 0:
                            ...
                            _, vaeLoss = self.sess.run(
                                [self.vae_optim2, self.vaeLoss2],
                                feed_dict={self.inputs: batch_images, self.z: batch_z})

                            data1 = self.sess.run(self.VAE1_generation,feed_dict={self.z:batch_z})
                            data = np.concatenate((data1[0:32],batch_images[0:32]),0)
                            _ = self.sess.run(
                                [self.vae_student],
                                feed_dict={self.inputs: data, self.z: batch_z})

                        elif taskIndex == 2:
                            _, vaeLoss = self.sess.run(
                                [self.vae_optim3, self.vaeLoss3],
                                feed_dict={self.inputs: batch_images, self.z: batch_z})

                            data1 = self.sess.run(self.VAE1_generation, feed_dict={self.z: batch_z})
                            data2 = self.sess.run(self.VAE2_generation, feed_dict={self.z: batch_z})

                            data = np.concatenate((data1[0:20], data2[0:20],batch_images[0:24]), 0)
                            _ = self.sess.run(
                                [self.vae_student],
                                feed_dict={self.inputs: data, self.z: batch_z})
                            ...
                            _, vaeLoss = self.sess.run(
                                [self.vae_optim4, self.vaeLoss4],
                                feed_dict={self.inputs: batch_images, self.z: batch_z})

                            data1 = self.sess.run(self.VAE1_generation, feed_dict={self.z: batch_z})
                            data2 = self.sess.run(self.VAE2_generation, feed_dict={self.z: batch_z})
                            data3 = self.sess.run(self.VAE3_generation, feed_dict={self.z: batch_z})

                            data = np.concatenate((data1[0:15], data2[0:15], data3[0:15],batch_images[0:19]), 0)
                            _ = self.sess.run(
                                [self.vae_student],
                                feed_dict={self.inputs: data, self.z: batch_z})
                            ...
                            _, vaeLoss = self.sess.run(
                                [self.vae_optim5, self.vaeLoss5],
                                feed_dict={self.inputs: batch_images, self.z: batch_z})

                            data1 = self.sess.run(self.VAE1_generation, feed_dict={self.z: batch_z})
                            data2 = self.sess.run(self.VAE2_generation, feed_dict={self.z: batch_z})
                            data3 = self.sess.run(self.VAE3_generation, feed_dict={self.z: batch_z})
                            data4 = self.sess.run(self.VAE4_generation, feed_dict={self.z: batch_z})

                            data = np.concatenate((data1[0:12], data2[0:12], data3[0:12], data4[0:12],batch_images[0:16]), 0)
                            _ = self.sess.run(
                                [self.vae_student],
                                feed_dict={self.inputs: data, self.z: batch_z})
            ...
            #Calculate individual
            index1 = 1
            mse1, ssim1, psnr1,log1 = self.Calculate_Criteria(self.mnist_test_x, index1)
            mse2, ssim2, psnr2,log2 = self.Calculate_Criteria(self.svhn_test_x, index1)
            mse3, ssim3, psnr3,log3 = self.Calculate_Criteria(self.FashionTest_x, index1)
            mse4, ssim4, psnr4,log4 = self.Calculate_Criteria(self.InverseFashionTest_x, index1)
            mse5, ssim5, psnr5,log5 = self.Calculate_Criteria(self.InverseMNISTTest_x, index1)
            ...
            sum4 = log1+log2+log3+log4+log5
            sum4 = sum4 / 5.0

            #mnist, Fashion, Svhn, InverseFashion, InverseMnist
            print(
                "MSE: %4.4f SSMI: %4.4f, PSNR: %.8f, LOG: %.8f" \
                % (mse1, ssim1, psnr1,log1))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f, LOG: %.8f" \
                  % (mse2, ssim2, psnr2,log2))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f, LOG: %.8f" \
                  % (mse3, ssim3, psnr3,log3))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f, LOG: %.8f" \
                  % (mse4, ssim4, psnr4,log4))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f, LOG: %.8f" \
                  % (mse5, ssim5, psnr5,log5))
            print('\n')
            print("MSE: %4.4f SSMI: %4.4f, PSNR: %.8f, LOG: %.8f" \
                  % (sum1, sum2, sum3,sum4))

            self.saver.save(self.sess, "models/BatchEnsemble_Student_Unsupervised")
