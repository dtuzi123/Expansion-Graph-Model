...
parser.add_argument("--objective", type=str, default="iwae_elbo",
...
for runIndex in range(RunCount):
    ...
    if args.stochastic_layers == 1:
        ...
        n_latent = [100, 50]
        n_hidden = [200, 100]
        model = InfiniteVAEMixture_Weighted.InfiniteVAEMixture_Weighted(n_hidden, n_latent)
    ...
    for taskIndex in range(taskCount):
        ...
        if isGenerate == True:
            currentX = np.concatenate((currentX,x_from_prior),axis=0)
        ...
        threshold = 0.001
        if taskIndex == 0:
            evaluatedX = arr2[0:batch_size * 10]
        elif taskIndex == 1:
            evaluatedX = arr3[0:batch_size * 10]
        elif taskIndex == 2:
            evaluatedX = arr4[0:batch_size * 10]
        elif taskIndex == 3:
            evaluatedX = arr5[0:batch_size * 10]

        minIndex, minvalue = model.ExpansionAndSelection(evaluatedX, 5000)
        ...
        if threshold < minvalue:
            ...
            currentNet = model.AllNodeArr[minIndex]
            # generate images
            prior = tfd.Normal(0, 1)
            prior_samples = prior.sample((64 * 100, 100))
            x_from_prior = currentNet.Generate_From_Prior(prior_samples)
            x_from_prior = np.reshape(x_from_prior,(-1,28*28))
            isGenerate = True
