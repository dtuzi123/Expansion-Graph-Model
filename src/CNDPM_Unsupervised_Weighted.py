...
class VAENode(tf.keras.Model):
    ...
    def call(self, x, n_samples, beta=1.0):
        ...
        if self.IsBasic == True:
            ...
            qzx = tfd.Normal(q_mu, q_std + 1e-6)
            z = qzx.sample(n_samples)
            z_sum = z #* self.ComponentWeights[i]
...
class CNDPM_Unsupervised_Weighted(tf.keras.Model):
    def __init__(self,
                 n_hidden,
                 n_latent,
                 **kwargs):
        super(CNDPM_Unsupervised_Weighted, self).__init__(**kwargs)
    ...
    def Create_New_Component(self,x,data,isFirst):
        ...
        if isFirst == True:
            ...
            arr = []
            isExpansion = False
            print("build a component")

            # calculate normal node
            newModel = VAENode(200, False)
            newModel.Build_NormalNode(x, self.BasicNodeArr, self.basic_number, 0, self.n_samples)
