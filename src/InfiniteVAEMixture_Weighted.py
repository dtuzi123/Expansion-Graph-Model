...
class VAENode(tf.keras.Model):
    ...
    def Generate_From_Prior(self,z):
        generated = 0
        if self.IsBasic == True:
            ...
            generated = pxz.sample(1)
        else:
            decoderOutput = self.BasicNodes[0].SharedDecoder(z)
            decoderOutput = self.SpecificDecoder_layer1(decoderOutput)
            logits = self.SpecificDecoder_output(decoderOutput)
            pxz = tfd.Bernoulli(logits=logits)
            generated = pxz.sample(1)
    ...
    def call(self, x, n_samples, beta=1.0):
        ...
        if self.IsBasic == True:
            ...
            for i in range(basicCount):
                ...
                lqzx = tf.reduce_sum(qzx.log_prob(z), axis=-1)
                kl = (lpz - lqzx)
                kl_sum = kl_sum + kl # self.ComponentWeights[i] * kl
    ...
    def Build_BasicNode(self,x,n_samples):
        ...
        decoderOutput = self.SharedDecoder(z)
        decoderOutput =Build_BasicNode self.SpecificDecoder_layer1(decoderOutput)
        logits = self.SpecificDecoder_output(decoderOutput)
...
class InfiniteVAEMixture_Weighted(tf.keras.Model):
    def __init__(self,
                 n_hidden,
                 n_latent,
                 **kwargs):
        super(InfiniteVAEMixture_Weighted, self).__init__(**kwargs)
    ...
    def ExpansionAndSelection(self,textX,n_samples):

        prior = tfd.Normal(0,1)
        prior_samples = prior.sample((64*10,100))
        arr = []
        for i in range(np.shape(self.AllNodeArr)[0]):
            x_from_prior = self.AllNodeArr[i].Generate_From_Prior(prior_samples)
            x_from_prior = tf.reshape(x_from_prior,(64*10,28*28))
            loss2 = self.Calculate_NLL_By_SelectedComponent(i, n_samples, x_from_prior)
            loss = self.Calculate_NLL_By_SelectedComponent(i, n_samples, textX)
            diff = tf.abs(loss - loss2)
            arr.append(diff)
        arr = np.array(arr)
        print(arr)
        minvalue = np.min(arr)
