...
import utils
from Utils2 import *
import CNDPM_Unsupervised_Weighted
...
if args.stochastic_layers == 1:
    ...
    model = CNDPM_Unsupervised_Weighted.CNDPM_Unsupervised_Weighted(n_hidden[0], n_latent[0])
    model.threshold = 100
    #model.Create_New_Component(myInput,0)
else:
    n_latent = [100, 50]
    n_hidden = [200, 100]
    model = CNDPM_Unsupervised_Weighted.CNDPM_Unsupervised_Weighted(n_hidden, n_latent)


myInput = tf.keras.layers.Input(shape=(28*28,))

learning_rate_dict[0] = 0.0001
optimizer = tensorflow.keras.optimizers.Adam(learning_rate_dict[0], epsilon=1e-4)
