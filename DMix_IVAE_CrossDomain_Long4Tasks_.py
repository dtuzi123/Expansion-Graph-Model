...
if args.stochastic_layers == 1:
    ...
    model = DMix.DMix(n_hidden[0], n_latent[0])
    model.threshold = 100
...
for taskIndex in range(taskCount):
    ...
    for epoch in range(epochs):
        ...
        g1 = np.reshape(Xtrain_binarized[0:64],(-1,28,28,1))
        ims("results/" + "aaaa.png", merge2(g1[:64], [8, 8]))
...
L = 5000
index1,value1 = model.Evaluation(mnistTest,L)
index2,value2 = model.Evaluation(fashionTest,L)
index3,value3 = model.Evaluation(CaltechTesting,L)
index4,value4 = model.Evaluation(omnistTestingSet,L)
